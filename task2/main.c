...
int main()
{
    ...
    for(int i = 0; i < 27; i++)
    {
        if(arr_big[i]!=0)
        {
            arr_small[count2++] = arr_big[i];	/*写入当前数组位置，并且将count2指向下一个数组位置*/
        }
    }
    ...
    {
        int length = 0;
        if(huffman[i][1] == -1 && huffman[i][2] == -1)  /*判断存在*/
        {
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    for (int i = 0; i < 2 * n - 1; i++)
    {
        if (huffman[i][3] == -1 && huffman[i][0] == -1)  /*补断是否有父*/
        {
            break;
        }
        if (huffman[i][3] < min && huffman[i][0] == -1)  /*找到权值最小*/
        {
            min = huffman[i][3];
            ii[0] = i;
        }
    ...
    for (int i = 0; i < 2 * n - 1; i++)
    {
        if (huffman[i][3] == -1 && huffman[i][0] == -1)  /*判断是否有父*/
        {
            break;
        }
        if (huffman[i][3] < min2 && huffman[i][0] == -1 && i != ii[0])  /*补找到权值最小*/
        {
            min2 = huffman[i][3];
            ii[1] = i;
        }
